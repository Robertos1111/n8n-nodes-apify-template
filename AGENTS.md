...
1. **Template Files** (`nodes/ApifyFacebookCommentsScraper/`) - Blueprint for generated nodes
...
Location: `nodes/ApifyFacebookCommentsScraper/`

These files serve as the blueprint for all generated nodes:

### Node Definition
- **`ApifyFacebookCommentsScraper.node.ts`** - Main node class implementing `INodeType`
...
### Properties
- **`ApifyFacebookCommentsScraper.properties.ts`** - UI field definitions
...
### Metadata
- **`ApifyFacebookCommentsScraper.node.json`** - Node categories and aliases
...
- Renames `ApifyFacebookCommentsScraper/` → `ApifyActorName/`
...
generateActorResources() → Convert Apify schema to n8n properties
      ↓
refactorProject() → Rename ApifyFacebookCommentsScraper → ApifyInstagramScraper
      ↓
...
| File | Purpose |
|------|---------|
| `nodes/ApifyFacebookCommentsScraper/ApifyFacebookCommentsScraper.node.ts` | Main node template |
| `nodes/ApifyFacebookCommentsScraper/helpers/executeActor.ts` | Actor execution logic |
| `nodes/ApifyFacebookCommentsScraper/helpers/genericFunctions.ts` | API utilities |
...
Edit files in `nodes/ApifyFacebookCommentsScraper/` - changes will apply to all future generated nodes.
...
Edit `nodes/ApifyFacebookCommentsScraper/helpers/genericFunctions.ts` → modify `pollRunStatus()` function.

### Add Custom API Headers
Edit `nodes/ApifyFacebookCommentsScraper/helpers/genericFunctions.ts` → add headers in `apiRequest()` function.
...
- `ApifyFacebookCommentsScraper.properties.ts` - Completely rebuilt from Actor's input schema
- `properties.json` - Generated from converted schema

**Copied & modified (placeholders replaced):**
- `ApifyFacebookCommentsScraper.node.ts` - Class names and constants updated
- All helper files - Names and imports updated
- `ApifyFacebookCommentsScraper.node.json` - Display name updated
